...
`clusters/variants/pg-backups/`.

When it is `"true"` the cluster also requires `PG_BACKUP_DESTINATION`,
`PG_BACKUP_REGION`, `PG_BACKUP_RETENTION` and `PG_BACKUP_SCHEDULE`. Only
`clusters/variants/pg-backups/true/` reads them, so a cluster with backups off
does not need them and local/kind does not declare them. Nothing validates this.
